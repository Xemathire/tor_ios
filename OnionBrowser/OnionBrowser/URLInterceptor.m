...
- (instancetype)initWithRequest:(NSURLRequest *)request cachedResponse:(NSCachedURLResponse *)cachedResponse client:(id<NSURLProtocolClient>)client
{
    ...
    if ([[[[request URL] scheme] lowercaseString] isEqualToString:@"theonionbrowser"] || [[[[request URL] scheme] lowercaseString] isEqualToString:@"theonionbrowsers"]) {
        wvt = [[[appDelegate appWebView] webViewTabs] firstObject];
        
        // Attempting to load custom URL, allow
        return self;
    }
	...
	if (wvthash != nil && ![wvthash isEqualToString:@""]) {
		for (WebViewTab *_wvt in [[appDelegate appWebView] webViewTabs]) {
...
- (void)startLoading
{
    ...
    if ([[[[[self request] URL] scheme] lowercaseString] isEqualToString:@"theonionbrowser"]) {
        ...
        if ([[[[self request] URL] absoluteString] isEqualToString:@"theonionbrowser:home"]) {
...
- (void)HTTPConnection:(CKHTTPConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge
{
	...
	if (nsuc == nil) {
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			[[appDelegate appWebView] presentViewController:uiac animated:YES completion:nil];
		});
	}
	else {
		[[NSURLCredentialStorage sharedCredentialStorage] setCredential:nsuc forProtectionSpace:[challenge protectionSpace]];
