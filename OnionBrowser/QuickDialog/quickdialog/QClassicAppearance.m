...
- (UIView *)buildFooterForSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    if (self.sectionFooterFont!=nil && tableView.style == UITableViewStyleGrouped){
        ...
        NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:section.footer attributes:@{NSFontAttributeName: self.sectionFooterFont}];
        CGRect rect = [attributedText boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
...
- (CGFloat)heightForHeaderInSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    if (!tableView.root.grouped)  {
        // [section.title sizeWithFont:self.sectionTitleFont constrainedToSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)]
        NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:section.title attributes:@{NSFontAttributeName: self.sectionTitleFont}];
        CGRect rect = [attributedText boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
                                                   options:NSStringDrawingUsesLineFragmentOrigin
                                                   context:nil];
    ...
    if (section.title != nil) {
        ...
        //CGSize expectedLabelSize = [section.title sizeWithFont:appearance==nil? [UIFont systemFontOfSize:[UIFont labelFontSize]] : appearance.sectionTitleFont constrainedToSize:maximumLabelSize lineBreakMode:NSLineBreakByWordWrapping];
        NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:section.title attributes:@{NSFontAttributeName: appearance==nil? [UIFont systemFontOfSize:[UIFont labelFontSize]] : appearance.sectionTitleFont}];
...
- (CGFloat)heightForFooterInSection:(QSection *)section andTableView:(QuickDialogTableView *)tableView andIndex:(NSInteger)index {
    ...
    // [section.footer sizeWithFont:appearance.sectionFooterFont constrainedToSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)]
    NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:section.footer attributes:@{NSFontAttributeName: appearance.sectionFooterFont}];
    CGRect rect = [attributedText boundingRectWithSize:CGSizeMake(tableView.bounds.size.width-margin-margin, 1000000)
                                               options:NSStringDrawingUsesLineFragmentOrigin
                                               context:nil];
    CGSize size = rect.size;
