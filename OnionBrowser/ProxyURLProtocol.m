...
- (void)startLoading {
    if ([[[[[self request] URL] scheme] lowercaseString] isEqualToString:@"theonionbrowser"]) {
        ...
        if ([[[[self request] URL] absoluteString] rangeOfString:@"start2"].location != NSNotFound) {
            /* theonionbrowser:start2 -- inner iframe banner */
            ...
            /* theonionbrowser:help */
            url = [NSURL URLWithString: [NSString stringWithFormat:@"file:/%@/help.html",resourcePath]];
        } else if ([[[[self request] URL] absoluteString] rangeOfString:@"starting"].location != NSNotFound) {
            /* theonionbrowser:starting */
