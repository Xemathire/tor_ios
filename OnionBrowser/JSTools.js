...
// NOTE: Slightly modified to return only one element and to return the distance
;(function ($, undefined) {
...
function __TheOnionBrowerFindClosestLink(x, y) {
    var array, link, linkDistance = 50, image, imageDistance = 50, fuzz = 40;
    
    array = $.nearest({x: x, y: y}, "A");
    if (array.length == 2) {
        link = array[0];
        linkDistance = array[1];
    }
    
    array = $.nearest({x: x, y: y}, "IMG");
    if (array.length == 2) {
        image = array[0];
        imageDistance = array[1];
    }
    
    // alert($(image).width() + " — " + $(image).height())

    // Check distance to "fuzz" links to make them easier to click
    if ((imageDistance < linkDistance) && (imageDistance <= fuzz)) {
        return ["IMG", image];
    }
    
    if (linkDistance <= fuzz) {
        return ["A", link];
    }
...
function __TheOnionBrowerGetLinkInfoAtPoint(x,y) {
    var e = document.elementFromPoint(x,y), tag = "", link = "", jsonData = [];
    ...
    if (tag != "A" && tag != "IMG") {
        // If no link or image has been found, attempt to find one less than 40px away
        e = __TheOnionBrowerFindClosestLink(x, y);
        
        if (e && e[0].toUpperCase() == "A") {
            tag = "" + e[0];
            link = "" + e[1];
        } else if (e && e[0].toUpperCase() == "IMG") {
            tag = "" + e[0];
            link = "" + $(e[1]).src;
        }
    }
